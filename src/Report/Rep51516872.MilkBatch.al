...
Report 51516872 "Milk Batch"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Milk Batch.rdlc';
    ...
    dataset
    {
        dataitem("Salary Processing Buffer"; "Salary Processing Lines")
        {
                ...
                if "Salary Processing Buffer".USER = ProcessingUser then begin
                    ...
                    if Account.Get("Salary Processing Buffer"."Account No.") then begin
                        ...
                        if "Salary Processing Buffer"."Account No." = '350010' then begin
                            ...
                            GenJournalLine.Init;
                            GenJournalLine."Journal Template Name" := 'GENERAL';
                            GenJournalLine."Journal Batch Name" := 'MILK';
                            GenJournalLine."Line No." := LineNo;
                            GenJournalLine."Document No." := DocNo;
                        ...
                        or ("Salary Processing Buffer"."Account No." = '350010') then
                            ...
                            GenJournalLine.Description := 'Milk';
                        ...
                        and ("Salary Processing Buffer"."Account No." <> '350010') then begin
                            ...
                            if SittingAll = true then
                                ...
                                GenJournalLine.Description := 'Milk Processing Fee';
                            GenJournalLine.Amount := MilkFee;
                            GenJournalLine.Validate(GenJournalLine.Amount);
                            GenJournalLine."Bal. Account Type" := GenJournalLine."bal. account type"::"G/L Account";
                            GenJournalLine."Bal. Account No." := MilkGLAccount;
                            GenJournalLine.Validate(GenJournalLine."Bal. Account No.");
                            GenJournalLine."Shortcut Dimension 1 Code" := DActivity2;
                            GenJournalLine."Shortcut Dimension 2 Code" := DBranch2;
                            GenJournalLine.Validate(GenJournalLine."Shortcut Dimension 1 Code");
                       ...
                       GenJournalLine."Account No.":=AccountS."BOSA Account No";
                       GenJournalLine.VALIDATE(GenJournalLine."Account No.");
                       GenJournalLine.Description:='Deposit Contribution';
                       GenJournalLine.Amount:=-10000;
                       GenJournalLine.VALIDATE(GenJournalLine.Amount);
                       ...
                       GenJournalLine.INSERT;
                        ...
                        if SittingAll = false then begin
                            ...
                            if Account."Account Category" <> Account."account category"::Branch then
                                RunBal := ("Salary Processing Buffer".Amount - (MilkFee + ExciseFee));//+AvailableBal;

                            //Registration Fee
                            RegFee := 0;
                            Vend.Reset;
                            Vend.SetRange(Vend."No.", "Salary Processing Buffer"."Account No.");
                            Vend.SetFilter(Vend."Registration Date", '>%1', 20170820D);
                            ...
                            if Vend.Find('-') then begin
                                ...
                                //Charge Reg Fee
                                if (SureAccountCharges.FnCheckIfPaid("Salary Processing Buffer"."Account No.") = false) then begin
                                    LineNo := LineNo + 10000;
                                    SureAccountCharges.FnAccountOpen('GENERAL', 'MILK', LineNo, "Salary Processing Buffer"."Account No.", RegFee, DActivity2, DBranch2, DocNo);
                                end;
                            ...
                            //Loans.SETRANGE(Loans."Account No","Salary Processing Buffer"."Account No.");
                                ...
                                repeat
                                    if RunBal > 0 then begin
                                        ...
                                        if Loans."Oustanding Interest" > 0 then begin
                                            Interest := 0;
                                            //MESSAGE('%1',Loans."Oustanding Interest" );
                    ...
                    /*//**********************************Balance with salaries control A/C**************************************
                    ...
                    GenJournalLine."Posting Date":=PDate;
                    GenJournalLine."Account Type":=GenJournalLine."Account Type"::Vendor;
                    GenJournalLine."Account No.":=Vendor;
                    GenJournalLine.VALIDATE(GenJournalLine."Account No.");
                    GenJournalLine.Description:='Salary '+"Salary Processing Buffer"."Account No.";
                    GenJournalLine.Amount:="Salary Processing Buffer".Amount;
                    GenJournalLine.VALIDATE(GenJournalLine.Amount);
                    GenJournalLine.VALIDATE(GenJournalLine."Bal. Account No.");
                    GenJournalLine."Shortcut Dimension 1 Code":=DActivity2;
                    ...
                    //****************************End of Balance with salaries control A/C***************************************/
            ...
            begin
                ...
                /*IF Vendor = '' THEN
                  ERROR('You must specify Salaries Control account');*/          //Sal Control A/C
                ...
                //IssueDate:='..' + FORMAT(DMY2DATE(20,DATE2DMY(CALCDATE('-1M',TODAY),2),DATE2DMY(CALCDATE('-1M',TODAY),3)));


                //IF "Salary Processing Buffer".GETFILTER("Salary Processing Buffer"."Date Filter") = '' THEN
                //ERROR('Please specify the date filter.');


                GenJournalLine.Reset;
                GenJournalLine.SetRange(GenJournalLine."Journal Template Name", 'GENERAL');
                GenJournalLine.SetRange(GenJournalLine."Journal Batch Name", 'MILK');
                ...
                if DontChargeProcFee = false then begin
                    //IF SittingAll = FALSE THEN BEGIN
                    if Charges.Get('MILK') then begin
                        MilkGLAccount := Charges."GL Account";
                        MilkFee := Charges."Charge Amount";
                        ExciseFee := MilkFee * (Gensetup."Excise Duty(%)" / 100);
                    end;
                    //END;
                end;
             ...
             END;
               */
                ...
                StandingOrders.SETRANGE(StandingOrders.Status,StandingOrders.Status::"Pending Approval");
                IF StandingOrders.FIND('-') THEN
                ERROR('Please approve or reject all standing orders before salary processing.');
                */

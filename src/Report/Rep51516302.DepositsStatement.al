...
Report 51516302 "Deposits Statement"
{
    ...
    dataset
    {
        dataitem("Members Register"; Customer)
        {
            ...
            {
                DataItemLink = "Customer No." = field("No."), "Posting Date" = field("Date Filter");
                DataItemTableView = sorting("Transaction Type", "Loan No", "Posting Date") where("Transaction Type" = filter(Juja), Reversed = filter(false));
            ...
            {
                DataItemLink = "Customer No." = field("No."), "Posting Date" = field("Date Filter");
                DataItemTableView = sorting("Transaction Type", "Loan No", "Posting Date") where("Transaction Type" = const("PassBook Fee"), Reversed = filter(false));
                PrintOnlyIfDetail = false;
